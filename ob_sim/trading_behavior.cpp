...
int TradingBehavior::get_buysell_quantity(const int& order_submitted_imbalance) {
    // what we want is that reverse the order imbalance
    ...
    if (order_submitted_imbalance < 0) {
        buying_segment = probability_buying_splits[2];
    } else if (order_submitted_imbalance > 0) {
